        ...
        if code_act_config.enabled {
            ...
            if let Some(paths) = hive_code_executor::resolve_python_wasm(Some(&hivemind_home)) {
                ...
                std::thread::spawn(move || {
                    let _ = bus.publish(
                        "daemon.runtime.downloading",
                        "code_act",
                        serde_json::json!({ "runtime": "python-wasm", "message": "Downloading CodeAct runtime..." }),
                    );
                        ...
                        match hive_code_executor::ensure_python_wasm(&home) {
                                ...
                                match hive_code_executor::SessionRegistry::new_auto(
                                    ...
                                    Ok(registry) => {
                                        ...
                                        let _ = bus.publish(
                                            ...
                                            "code_act",
                                            serde_json::json!({ "runtime": "python-wasm", "message": "CodeAct runtime ready" }),
                                        );
                                        return;
                                    }
                                    Err(e) => {
                                        last_err = e.to_string();
                                        // Init failure is not retryable
                                        break;
                                    }
                                }
                            }
                            Err(e) => {
                                ...
                                tracing::warn!(
                                    ...
                                    error = %e,
                                    "CodeAct WASM download attempt failed"
                                );
                    ...
                    tracing::error!(error = %last_err, "CodeAct WASM runtime unavailable after all retries");
                    let _ = bus.publish(
                        "daemon.runtime.failed",
                        "code_act",
                        serde_json::json!({
                            "runtime": "python-wasm",
                            "message": format!("CodeAct runtime download failed: {last_err}"),
                            "error": last_err,
                        }),
                    );
                });
