    ...
    fn run<'a>(
        ...
        Box::pin(async move {
            ...
            loop {
                ...
                // Emit any events generated by middleware (e.g. compaction).
                if let Some(ref tx) = event_tx {
                    for hook in middleware {
                        for event in hook.drain_pending_events() {
                ...
                let response = if let Some(ref tx) = event_tx {
                    ...
                    let _ = tx.try_send(LoopEvent::ModelDone {
                        ...
                        model: model.clone(),
                        input_tokens: streamed_usage
                            .as_ref()
                            .map(|u| u.input_tokens)
                            .filter(|&t| t > 0),
                        output_tokens: streamed_usage
                            .as_ref()
                            .map(|u| u.output_tokens)
                            .filter(|&t| t > 0),
                        cached_input_tokens: streamed_usage
                            .as_ref()
                            .map(|u| u.cached_input_tokens)
                            .filter(|&t| t > 0),
                        cache_write_tokens: streamed_usage
                            .as_ref()
