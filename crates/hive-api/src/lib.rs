...
impl AppState {
    pub fn new(
        ...
        let plugin_host = {
            ...
            host =
                host.with_host_handler(Arc::new(move |method: &str, params: serde_json::Value| {
                    ...
                    Box::pin(async move {
                        match method.as_str() {
                            ...
                            "host/notify" => {
                                ...
                                tracing::info!(plugin_id, title, body, "Plugin notification");
                            ...
                            "host/schedule" => {
                                ...
                                let interval_secs =
                                    params["intervalSeconds"].as_u64().unwrap_or(60);
                                let plugin_id =
                                    params["pluginId"].as_str().unwrap_or("unknown").to_string();
                                ...
                                let request = CreateTaskRequest {
                                    ...
                                    schedule: TaskSchedule::Cron { expression: cron_expr },
                            ...
                            "host/unschedule" => {
                                ...
                                match task {
                                    Some(t) => match scheduler.cancel_task(&t.id) {
                                        Ok(_) => Ok(serde_json::json!({ "ok": true })),
                                        Err(e) => {
                                            anyhow::bail!("Failed to unschedule task: {}", e)
                                        }
                                    },
                                    None => Ok(serde_json::json!({ "ok": true, "found": false })),
                                }
                            }
                            _ => {
                        ...
                        Ok(change) => {
                            ...
                            let service_status = match change.status.state.as_str() {
                                ...
                                "disconnected" | "stopped" => {
                                    hive_contracts::ServiceStatus::Stopped
                                }
        ...
        // re-discover servers that failed due to missing Node.js / Python.
        {
            ...
            tokio::spawn(async move {
                ...
                loop {
                    ...
                    if pending_node {
                        server_ids.extend(
                            mcp.take_pending_servers_for_runtime(
                                hive_mcp::runtime::McpRuntime::Node,
                            ),
                        );
                    }
                    if pending_python {
                        server_ids.extend(mcp.take_pending_servers_for_runtime(
                            hive_mcp::runtime::McpRuntime::Python,
                        ));
                    }
                    pending_node = false;
    ...
    pub fn with_chat(
        ...
        Self {
            ...
            service_log_collector: hive_core::ServiceLogCollector::new(),
            service_registry: Arc::new(services::ServiceRegistry::new()),
            plugin_registry: Arc::new(hive_plugins::PluginRegistry::new(
