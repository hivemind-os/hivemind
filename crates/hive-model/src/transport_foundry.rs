//! Microsoft Foundry (Azure AI Foundry) transport implementation.
...
/// suffix from the configured base URL.
fn foundry_inference_url(base_url: &str) -> String {
    let base = trim_trailing_slash(base_url);
    let resource_base =
        if let Some(idx) = base.find("/api/projects/") { &base[..idx] } else { base };
    format!("{resource_base}/openai/v1/chat/completions")
}

pub(crate) struct FoundryTransport;

impl ProviderTransport for FoundryTransport {
    fn complete_blocking(
        ...
        let url = foundry_inference_url(ctx.base_url);
        ...
        let first_choice = response.choices.into_iter().next().ok_or_else(|| {
            anyhow!(
                "provider {} returned no choices in the azure foundry response body",
                ctx.provider_id
            )
        })?;
        ...
        if content.is_empty() && tool_calls.is_empty() {
            bail!(
                "provider {} returned no assistant content in the azure foundry response body",
                ctx.provider_id
            );
        }
