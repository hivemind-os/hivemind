...
export function startPluginRuntime<TShape extends ZodRawShape>(
  ...
  const scheduledHandlers = new Map<string, () => Promise<void>>();

  // ── Host → Plugin: initialize ───────────────────────────────
  transport.onMethod("initialize", async (params: any) => {
  ...
  transport.onMethod("plugin/validateConfig", async (params: any) => {
  ...
  transport.onMethod("plugin/activate", async (params: any) => {
    if (!context) throw new Error("Plugin not initialized");
    // Update config if provided
    if (params?.config) {
      (context as any)._config = params.config;
    }
  ...
  transport.onMethod("plugin/startLoop", async () => {
    ...
    if (loopAbortController) throw new Error("Loop already running");

    loopAbortController = new AbortController();
    const loopContext: PluginContext = {
      ...context,
  ...
  transport.onMethod("plugin/stopLoop", async () => {
    if (!loopAbortController) return { ok: true, wasRunning: false };
  ...
  transport.onMethod("tools/call", async (params: any) => {
    if (!context) throw new Error("Plugin not initialized");
  ...
  transport.onMethod("plugin/scheduledTask", async (params: any) => {
    const handler = scheduledHandlers.get(params?.id);
    if (!handler) throw new Error(`Unknown scheduled task: ${params?.id}`);
    await handler();
    return { ok: true };
  });
...
function createContext(
  ...
  hostInfo: { version: string; platform: string; capabilities: string[] },
): PluginContext {
  ...
  const ctx: PluginContext = {
    pluginId,
    get config() {
      return (ctx as any)._config ?? config;
    },
    signal: neverAbort.signal,

    // ── Messaging ───────────────────────────────────────────
    async emitMessage(msg: IncomingMessage): Promise<void> {
      await transport.request("host/emitMessage", { message: msg });
    },
    async emitMessages(msgs: IncomingMessage[]): Promise<void> {
    ...
    // ── Status ──────────────────────────────────────────────
    async updateStatus(status: PluginStatus): Promise<void> {
    ...
    async schedule(opts: ScheduleOptions): Promise<void> {
      ...
      (ctx as any)._scheduledHandlers?.set(opts.id, opts.handler);
    ...
    async unschedule(id: string): Promise<void> {
      (ctx as any)._scheduledHandlers?.delete(id);
    ...
    host: {
      ...
      platform: (hostInfo?.platform as any) ?? "linux",
  ...
  (ctx as any)._config = config;
  (ctx as any)._scheduledHandlers = new Map();
...
function buildCapabilities(definition: PluginDefinition<any>): string[] {
  const caps: string[] = ["tools"];
  if (definition.loop) caps.push("loop");
  if (definition.auth) caps.push("auth");
  if (definition.onActivate) caps.push("lifecycle");
  return caps;
}
  ...
  switch (typeName) {
    case "ZodObject": {
      ...
      for (const [key, fieldSchema] of Object.entries(shape)) {
        properties[key] = zodToJsonSchema(fieldSchema as any);
        if (!(fieldSchema as any)._def?.typeName?.includes("Optional") &&
            !(fieldSchema as any)._def?.typeName?.includes("Default")) {
          required.push(key);
        }
      }
