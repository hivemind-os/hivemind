/**
 * Borg Theme Visual Audit
 *
 * Systematically visits every page, dialog, tab, and overlay in the app
 * with the Borg theme active. Each test takes a screenshot so we can
...
/** Apply the Borg theme before the app reads localStorage */
async function applyBorgTheme(page: Page) {
  await page.addInitScript(() => {
    localStorage.setItem('hivemind-theme', 'borg');
  });
}
...
async function snap(page: Page, name: string) {
  ...
  await expect(page).toHaveScreenshot(`borg-${name}.png`, {
...
test.describe('Borg Theme — Full UI Audit', () => {
  test.beforeEach(async ({ page }) => {
    collectErrors(page);
    await applyBorgTheme(page);
